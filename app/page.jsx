...
export default function Home() {
  ...
  function getBotReply(input) {
    ...
    if (text.includes("gift") || text.includes("1500") || text.includes("1000")) {
      ...
      const pick = products.filter((product) => product.price > 0 && product.price <= budget).sort((a, b) => b.price - a.price)[0];
      return pick ? `Gift pick: ${pick.name} at ${formatPrice(pick.price)}. It is an easy, premium choice within Rs ${budget.toLocaleString("en-IN")}.` : "Try a discovery set for gifting when you are unsure about notes.";
    }
    if (text.includes("checkout") || text.includes("cart")) {
      return cart.length ? `Your cart has ${cart.length} item${cart.length > 1 ? "s" : ""} worth ${formatPrice(subtotal)}.` : "Your cart is empty. Start with Amber Muse or the Discovery Scent Wardrobe.";
    }
  ...
  return (
    <>
      ...
      <main id="top">
        <HeroSection content={storefront.hero} featuredProducts={products.slice(0, 3)} />
        ...
        <ProductShowcase content={storefront.products} filters={storefront.filters} filter={filter} setFilter={setFilter} products={visibleProducts} addToCart={addToCart} />
